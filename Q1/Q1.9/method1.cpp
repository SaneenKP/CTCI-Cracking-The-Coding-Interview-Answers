...
int main(){
    ...
    //hence to check if s2 is a rotation of s1.
    //s2 will always be the substring of s1+s1 if its a rotation.
    std::cout<<((isSubString(s1+s1 , s2)) || (s1.length() != s2.length()) ? "True" : "False");
}